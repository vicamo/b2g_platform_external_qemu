...
#define LENGTH_CHECK(cmd)	if (length < sizeof(cmd##_cp)) goto short_hci
    ...
    case cmd_opcode_pack(OGF_LINK_CTL, OCF_AUTH_REQUESTED):
        ...
        uint16_t auth_req_handle = 0;
        if (PARAMHANDLE(auth_requested)) {
            auth_req_handle = PARAMHANDLE(auth_requested);
        } else {
            auth_req_handle = hci->lm.last_handle | HCI_HANDLE_OFFSET;
        }
        if (bt_hci_handle_bad(hci, auth_req_handle)) {
            bt_hci_event_status(hci, HCI_NO_CONNECTION);
        } else {
            bt_hci_event_status(hci, HCI_SUCCESS);
            bt_hci_request_link_key(hci, auth_req_handle);
        }
        break;
    ...
    case cmd_opcode_pack(OGF_LINK_CTL, OCF_LINK_KEY_NEG_REPLY):
        ...
        bt_hci_event_complete(hci, &PARAM(link_key_reply, bdaddr),
    ...
    case cmd_opcode_pack(OGF_LINK_CTL, OCF_IO_CAPABILITY_REQ_REPLY):
        ...
        bt_hci_event_complete(hci, &PARAM(io_capability_req_reply, bdaddr),
            IO_CAPABILITY_REQ_REPLY_CP_SIZE);
        // The remote I/O capability exchangement has been done.
        // Here we assume that the remote device has the same I/O capability as
        // local. If we expect to emulate the different I/O capability of remote
        // device in the future, we should response with corresponding parameters.
    ...
    case cmd_opcode_pack(OGF_LINK_CTL, OCF_USER_CONFIRMATION_REQ_REPLY):
        ...
        bt_hci_event_complete(hci, &PARAM(user_confirmation_req_reply, bdaddr),
             USER_CONFIRMATION_REQ_CP_SIZE);
        ...
        for (i = 0; i < HCI_HANDLES_MAX; i ++)
            if (hci->lm.handle[i].link &&
                !bacmp(&hci->lm.handle[i].link->slave->bd_addr,
                    &PARAM(user_confirmation_req_reply, bdaddr)))
            {
                bt_hci_event_auth_complete(hci, hci->lm.handle[i].link->handle);
                break;
            }
        break;
    ...
    case cmd_opcode_pack(OGF_HOST_CTL, OCF_DELETE_STORED_LINK_KEY):
        ...
        bt_hci_request_delete_link_key(hci,
            &PARAM(delete_stored_link_key, bdaddr));
        break;
...
{
    static delete_stored_link_key_rp params;
    params.status = HCI_SUCCESS;
    params.num_keys = 1;

    bt_hci_event_complete(hci, &params, DELETE_STORED_LINK_KEY_RP_SIZE);
}
