...
void
console_log_msg(const char* fmtstr, ...)
{
    char str[256];
    va_list ap;

    va_start(ap, fmtstr);
    vsnprintf(str, sizeof(str), fmtstr, ap);
    va_end(ap);

    control_write(Client, str);
}

void
console_log_err(const char* fmtstr, ...)
{
    char str[256];
    va_list ap;
...
{
    ControlClient current_client;
    int res;

    current_client = Client;
    Client = client;

    res = nfc_cmd_snep(args);
